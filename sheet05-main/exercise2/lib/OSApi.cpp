...
int getRandomNumber() {
    // Seed the random number generator (call this once before using std::rand)
    std::srand(static_cast<unsigned>(std::time(nullptr)));
    return std::rand();
}
int createFile(const char* path)
// Create a file at a given path. Returns the file descriptor of the file.
{
...
void removeFile(const char* path) {
    // Use the correct path to remove the file
    if (unlink(path) < 0) {
